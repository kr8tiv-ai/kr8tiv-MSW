...
<task type="auto">
  <name>Add test:e2e:health script and stub test file</name>
  <files>package.json, tests/e2e/health-check.test.ts</files>
  <action>
Add npm script for E2E health check and create stub test file:
...
**Create stub test file: tests/e2e/health-check.test.ts**
```typescript
...
Note: Test is skipped until Phase 2 implementation. The workflow infrastructure is in place and won't fail - it will show as "skipped" until the actual test is implemented.
  </action>
  <verify>
cat package.json | grep "test:e2e:health"
ls tests/e2e/health-check.test.ts
cat tests/e2e/health-check.test.ts | grep "it.skip"
  </verify>
  <done>package.json has test:e2e:health script and tests/e2e/health-check.test.ts stub exists with it.skip placeholder</done>
</task>
