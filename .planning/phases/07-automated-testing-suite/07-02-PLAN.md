...
describe("ConfigManager", () => {
  ...
  describe("validate", () => {
    ...
    it("validates schema constraints on load", async () => {
      const configPath = path.join(testDir, "config.json");
      const invalidConfig = {
        version: "0.1.0",
        notebookUrl: "https://notebooklm.google.com/notebook/abc123",
        relevanceThreshold: 200, // Invalid: > 100
      };
      fs.writeFileSync(configPath, JSON.stringify(invalidConfig));

      // Should throw on load due to schema validation
      await expect(configManager.load()).rejects.toThrow("relevanceThreshold");
    });
  });
});
```
