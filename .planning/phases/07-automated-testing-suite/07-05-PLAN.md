...
describe("NotebookLM upload E2E", () => {
  ...
  it("handles MCP tool errors gracefully", async () => {
    ...
    // Test missing config
    const missingConfigResult = await tc.client.callTool({
      name: "msw_research",
      arguments: {
        projectDir: "/tmp/no-config-here",
      },
    });

    const missingConfigData = JSON.parse(missingConfigResult.content[0].text);
    expect(missingConfigData.success).toBe(false);
    expect(missingConfigData.error).toContain("config");
  });
});
```
...
- MCP tool error handling (invalid project dir, missing config)
  </action>
  <verify>
```bash
